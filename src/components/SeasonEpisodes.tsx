...
import { formatRuntime, type MediaItem } from "@/lib/plex-data";
  ...
  async function changeSeason(seasonKey: string) {
    ...
    try {
      const res = await fetch(
        `/api/plex/library/metadata/${encodeURIComponent(seasonKey)}/children`,
      );
      const data = await res.json();
      const items: RawEpisode[] = data?.MediaContainer?.Metadata ?? [];
      setEpisodes(
        items.map((d) => ({
